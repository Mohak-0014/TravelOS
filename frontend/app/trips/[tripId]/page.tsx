...
// ── Agent Activity collapsible panel ─────────────────────────────────────────
  ...
  return (
    <div className="glass-card overflow-hidden">
      ...
      >
        ...
        <ChevronRight
          className={`w-3.5 h-3.5 text-slate-600 transition-transform ${open ? "rotate-90" : ""}`}
        />
      </button>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="px-4 pb-4 space-y-1.5 max-h-48 overflow-y-auto border-t border-white/6 pt-3">
              {messages.slice(-12).map((msg, i) => (
                <p key={i} className="text-[10px] text-slate-500 leading-relaxed">
                  <span className="text-electric-500 font-medium">{msg.role}:</span>{" "}
                  {msg.content}
                </p>
              ))}
            </div>
