...
import { Plane, ArrowRight, ArrowDown, Star } from "lucide-react";
import SkyScene from "@/components/travel/SkyScene";

// ── Agent pipeline data ───────────────────────────────────────────────────────

const AGENTS = [
  { emoji: "🧭", name: "Travel Style", desc: "Learns your pace, taste, and style", color: "from-electric-400 to-electric-600", glow: "rgba(54,163,224,0.3)" },
  { emoji: "🗺️", name: "Itinerary", desc: "Clusters attractions into walkable zones", color: "from-coral-400 to-coral-600", glow: "rgba(255,107,92,0.3)" },
  { emoji: "🏨", name: "Hotel Agent", desc: "Ranks hotels by your luxury tier + budget", color: "from-gold-400 to-gold-500", glow: "rgba(245,166,35,0.3)" },
  { emoji: "💸", name: "Budget", desc: "Catches overspend. Suggests upgrades.", color: "from-emerald-400 to-emerald-500", glow: "rgba(31,169,122,0.3)" },
  { emoji: "🎭", name: "Local Events", desc: "Finds what's on during your trip", color: "from-coral-400 to-gold-400", glow: "rgba(255,138,115,0.3)" },
  { emoji: "🌤️", name: "Weather", desc: "Replans around rain before you notice", color: "from-electric-400 to-electric-500", glow: "rgba(91,184,232,0.3)" },
  { emoji: "✅", name: "Approval Gate", desc: "You review, you decide — AI proposes, not imposes", color: "from-emerald-400 to-electric-400", glow: "rgba(52,199,154,0.3)" },
  { emoji: "🧳", name: "Memory", desc: "Every trip makes the next one better", color: "from-gold-400 to-coral-400", glow: "rgba(245,166,35,0.3)" },
];

// ── Feature highlights ────────────────────────────────────────────────────────

const FEATURES = [
  { icon: "🎯", title: "Personalization", desc: "Remembers you love rooftop bars. Knows you hate museums before noon. Gets sharper with every trip.", color: "from-electric-400 to-electric-600", border: "hover:border-electric-500/40", glow: "hover:shadow-[0_24px_50px_rgba(54,163,224,0.18)]" },
  { icon: "⚡", title: "Live Adaptation", desc: "Rain forecast on Day 3? We've already moved the outdoor activity indoors. No notification needed.", color: "from-gold-400 to-gold-500", border: "hover:border-gold-400/40", glow: "hover:shadow-[0_24px_50px_rgba(245,166,35,0.18)]" },
  { icon: "💡", title: "Budget Intelligence", desc: "Over by 22%? Here's what to swap. Under by 30%? Here's an upgrade that fits perfectly.", color: "from-coral-400 to-coral-600", border: "hover:border-coral-500/40", glow: "hover:shadow-[0_24px_50px_rgba(255,107,92,0.18)]" },
];
...
const DEMO_DAYS = [
  ...
  { day: 2, title: "Harajuku", icon: "🎌", desc: "Takeshita St & Meiji", color: "from-coral-400 to-gold-400" },
  ...
  return (
    <div ref={ref} className="hidden lg:block absolute top-9 left-0 right-0 pointer-events-none" style={{ zIndex: 0 }}>
      ...
      <motion.div
        ...
        style={{ scaleX: dash, background: "linear-gradient(90deg,#36a3e0,#ff6b5c,#f5a623)" }}
      />
      <motion.div className="absolute top-0" style={{ left: dist }}>
        <Plane className="w-6 h-6 text-coral-500 -translate-x-1/2 -translate-y-1/2 rotate-90 drop-shadow-[0_4px_8px_rgba(20,34,61,0.18)]" fill="#ff6b5c" />
      </motion.div>
    </div>
  );
}
  ...
  return (
    <div className="relative inline-flex items-center justify-center">
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        <circle cx={50} cy={50} r={r} fill="none" stroke="rgba(20,34,61,0.08)" strokeWidth={stroke} />
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
        <span className="text-sm font-bold text-coral-600">73%</span>
    ...
    <div className="relative bg-space-900 overflow-x-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <motion.div style={{ y: contentY, opacity: heroFade }} className="relative z-10 text-center px-4 max-w-4xl mx-auto">
          ...
          >
            <span className="inline-flex items-center gap-2 text-xs font-semibold px-4 py-2 rounded-full glass electric-border text-electric-600 uppercase tracking-widest">
              <span className="w-1.5 h-1.5 rounded-full bg-coral-500 animate-pulse-glow" />
          ...
          >
            ...
            <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} onClick={() => scrollToSection("how-it-works")} className="btn-ghost flex items-center gap-2 text-sm px-6 py-3.5">
              See how it works
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 z-10">
          <div className="w-px h-8 bg-gradient-to-b from-transparent to-coral-500/60 animate-float-slow" />
          <div className="w-1.5 h-1.5 rounded-full bg-coral-500/60" />
        </motion.div>
      </section>

      {/* ════════════════ SECTION 2 — HOW IT WORKS ════════════════ */}
      <section id="how-it-works" className="relative py-28 px-4 overflow-hidden bg-space-900">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-px bg-gradient-to-r from-transparent via-coral-500/25 to-transparent" />
        ...
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.55, ease: [0.22, 1, 0.36, 1] }} className="text-center mb-16">
            <p className="text-xs font-semibold text-coral-600 uppercase tracking-[0.2em] mb-4">The Itinerary Engine</p>
              ...
              {AGENTS.map((agent, i) => (
                <motion.div
                  ...
                  onMouseEnter={(e) => {
                    ...
                    el.style.boxShadow = `0 18px 40px ${agent.glow}, 0 8px 20px rgba(20,34,61,0.08)`;
                    el.style.borderColor = agent.glow.replace("0.3", "0.45");
                  }}
                  onMouseLeave={(e) => {
                    const el = e.currentTarget;
                    el.style.boxShadow = "";
                ...
                >
                  ...
                  <div className="mt-auto w-5 h-5 rounded-full bg-space-800 border border-ink-900/10 flex items-center justify-center">
      ...
      <section className="relative py-24 px-4 bg-space-800">
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          <div className="absolute top-1/2 left-1/4 w-96 h-96 rounded-full bg-coral-500/8 blur-[120px] -translate-y-1/2" />
          <div className="absolute top-1/2 right-1/4 w-96 h-96 rounded-full bg-electric-500/8 blur-[120px] -translate-y-1/2" />
        </div>
        ...
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.55 }} className="text-center mb-14">
            <p className="text-xs font-semibold text-gold-500 uppercase tracking-[0.2em] mb-4">What Makes It Different</p>
      ...
      {/* ════════════════ SECTION 4 — DEMO + CTA ════════════════ */}
      <section className="relative py-28 px-4 overflow-hidden bg-space-900">
        <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="max-w-5xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.55 }} className="text-center mb-14">
            <p className="text-xs font-semibold text-coral-600 uppercase tracking-[0.2em] mb-4">See It In Action</p>
          ...
          <motion.div initial={{ opacity: 0, y: 32 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }} className="glass-card p-6 md:p-8 mb-12">
            <div className="flex items-start justify-between gap-4 mb-8 flex-wrap">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="status-badge text-emerald-600 bg-emerald-400/15 border border-emerald-400/30">Ready</span>
                </div>
            ...
            <div className="glass-light rounded-xl px-5 py-3.5 flex items-center gap-3">
              ...
              <div className="ml-auto flex items-center gap-1">
                {[1, 2, 3, 4, 5].map((s) => (
                  <Star key={s} className="w-3.5 h-3.5 text-gold-500" fill="#f5a623" />
                ))}
              </div>
            </div>
          </motion.div>
      ...
      {/* ════════════════ SECTION 5 — FOOTER ════════════════ */}
      <footer className="relative border-t border-slate-800 py-12 px-4 bg-space-800">
        <div className="max-w-5xl mx-auto flex flex-col items-center gap-6 text-center">
          ...
          <div className="flex items-center gap-6">
            <Link href="/login" className="text-xs text-slate-500 hover:text-coral-600 transition-colors">Sign In</Link>
            <span className="text-slate-700">·</span>
            <Link href="/trips" className="text-xs text-slate-500 hover:text-coral-600 transition-colors">My Trips</Link>
            <span className="text-slate-700">·</span>
            <Link href="/profile" className="text-xs text-slate-500 hover:text-coral-600 transition-colors">Profile</Link>
          </div>
        </div>
      </footer>
    </div>
